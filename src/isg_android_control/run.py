...
async def mqtt_worker() -> None:
    ...
    cec_ha = None
    if monitor and monitor.cec_controller:
        try:
            cec_ha = CECHAIntegration(ha, monitor.cec_controller)
            logger.info("CEC Home Assistant integration initialized")
        except Exception as e:
            logger.warning("Failed to initialize CEC HA integration: %s", e)
    ...
    def _on_connect(client, userdata, flags, rc):
        try:
            ...
            # Setup CEC subscriptions if available
            if cec_ha:
                asyncio.create_task(cec_ha.setup_subscriptions())
        except Exception:
            pass
    ...
    # Publish CEC discovery if available
    if cec_ha:
        try:
            await cec_ha.publish_discovery()
            await cec_ha.setup_subscriptions()
            logger.info("Published CEC discovery to Home Assistant")
        except Exception as e:
            logger.warning("Failed to publish CEC discovery: %s", e)
    ...
    async def publisher() -> None:
        ...
        while True:
            try:
                ...
                # Add CEC state publishing if available
                if cec_ha:
